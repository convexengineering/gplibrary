...
class Atmosphere(Model):
    ...
    def __init__(self, N, **kwargs):
        ...
        index = map(int, np.linspace(0, N-1, N))
        ...
        rho_0  = Variable('rho_0', 'kg/m^3', 'Density at sea level')
...
##            rhou*kgm3 == rho,
            ...
            #altitudes corresponding to discretized flight profile
            h[0] == 1104.9*units('m'),
            h[1] == 2400.3*units('m'),
            h[2] == 4572.0*units('m'),
            h[3] == 7620.0*units('m'),
            ...
            #correct pressures for altitudes given above
##            p[0]==88.73*units('kPa'),
##            p[1]==75.611*units('kPa'),
##            p[2]==57.164*units('kPa'),
##            p[3]==37.58*units('kPa'),
##            p[4]==30.07*units('kPa'),
##            p[5]==30.07*units('kPa'),
...
##                                   (hu[i])**0.933)),
                        ...
                        #simple temperature lapse rate model
                       SignomialEquality(T[i] + L*h[i], T_0),

                       #various atmosphere constraints
                       
                       (p[i]/p_0) == (T[i]/T_0)**th,
                       
##                       TCS([T_0 >= T[i] + L*h[i]]),
                       
##                       (rho[i]/rho_0) == (T[i]/T_0)**(th-1)
                        ])

        Model.__init__(self, sum(T), constraints, **kwargs)
