...
class StationKeeping(Model):
    def __init__(self, **kwargs):
        ...
        h_min = Variable('h_{min}', 15000, 'ft', 'minimum altitude for coverage requirement')
        h_max = Variable('h_{max}', 11000, 'm', 'maximum altitude for GP fit')
	...
	with SignomialsEnabled():
	    constraints = [h >= h_min,
                       (6.39e-27 * (h/h_0)**13.7
                        + 7.65e-27 * (h/h_0)**13.7
                        + 2.81e-09 * (h/h_0)**8.91)/(V/V_0)**(17.8) <= 1,
                       h <= h_max] # units?
        Model.__init__(self, None, constraints, **kwargs)
