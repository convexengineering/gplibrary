...
class DummyAirfoil(Model):
   ...
   A better version of this model would be to run GPfit on MSES data.
    ...
    def setup(self, flight_state):
        ...
        self.mach_perp = mach_perp = Variable('mach_perp', '', 'Mach number of velocity component perpendicular to leading edge')
        self.c_l = c_l = Variable('c_l', '', '2D Lift coefficient')
        self.c_dp = c_dp = Variable('c_dp', '', '2D Drag coefficient due to pressure')
        self.c_df = c_df = Variable('c_df', 50e-4, '', '2D Drag coefficient due to friction')
        self.c_la_2d = c_la_2d = Variable('c_la_2d', 'radian**-1', 'Airfoil 2d d C_L/ d alpha')

        # Variables internal to the model
        self._c_dp0 = _c_dp0 = Variable('_c_dp0', '', '2D Drag coefficient due to pressure at low Mach number')
        self._mach_d = _mach_d = Variable('_mach_d', '', 'Drag divergence Mach number')
        self._mach_model_max = _mach_model_max = Variable('_mach_model_max', 0.8, '', 'Maximum Mach number at which this model is valid')
        self._c_l_model_max = _c_l_model_max = Variable('_c_l_model_max', 1.1, '', 'Maximum lift coefficient at which this model is valid')
