...
class Power(Model):
    def __init__(self, N, latitude, day, **kwargs):
        ...
        eta_charge = Variable("\\eta_{charge}", 0.95, "-",
                              "Battery charging efficiency")
        eta_discharge = Variable("\\eta_{discharge}", 0.95, "-",
...
class SolarSimple(Model):
    def __init__(self, latitude=45, avail=90, day=355, h_station=16000, N=1, **kwargs):

        slf = SteadyLevelFlight(N, [0.7]*N)
        aero = Aerodynamics(N, jh01=False)
        atm = Atmosphere(N, [h_station]*N)
